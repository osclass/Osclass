...
?>
    <div id="category" style="width: <?php echo $width ?>;">
        <div class="category_header"><a href="<?php osc_search_category_url($c, true); ?>"><?php echo $c['s_name']; ?></a> <span style="color: #ccc;">[<?php echo CategoryStats::newInstance()->getNumItems($c); ?>]</span></div>
        <div>
        <?php foreach($c['categories'] as $sc): ?>
            <div class="category_body"><a href="<?php osc_search_category_url($sc, true); ?>"><?php echo $sc['s_name']; ?></a> <span style="color: #ccc;">[<?php echo CategoryStats::newInstance()->getNumItems($sc); ?>]</span></div>
        <?php endforeach; ?>
        </div>
    </div>
    <?php if ($i == $colnums) { ?>
