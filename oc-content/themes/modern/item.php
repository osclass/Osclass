	...
	<div id="item_desc_body">
		...
		<div id="item_images">
			<?php if(count($resources)): ?>
				...
				<?php foreach($resources as $r): ?>
					<img src="<?php echo osc_createResourceURL($r); ?>" style="width: 400px; border: 1px solid #ccc;" /><br />
				<?php endforeach; ?>
			<?php else: ?>
		...
		<div id="item_desc">
			...
			<div style="-moz-border-radius: 4px; border: 1px solid white; background-color: #eee; padding: 20px; margin-top: 10px; font-family: Tahoma; font-size: 14px;">
                            ...
                            <div style="padding: 2px;"><?php _e('City area'); ?> : <?php echo $item['s_city_area'] ?></div>
                            <div style="padding: 2px;"><?php _e('Address'); ?> : <?php echo $item['s_address'] ?></div>
			</div>
			
			<!-- plugins -->
			<?php
			osc_run_hook('item_detail', $item);
			?>
