<?php if ( ! defined('ABS_PATH')) exit('ABS_PATH is not loaded. Direct access is not allowed.');
    ...
    class Stats extends DAO {
        ...
        public function new_users_count($from_date) {
            return $this->conn->osc_dbFetchResults("SELECT DATE(dt_reg_date) as d_date, COUNT(pk_i_id) as num FROM %st_user WHERE dt_reg_date > '%s' GROUP BY DAY(dt_reg_date) ORDER BY dt_reg_date ASC", DB_TABLE_PREFIX, $from_date);
        }
        ...
        public function new_items_count($from_date) {
            return $this->conn->osc_dbFetchResults("SELECT DATE(dt_pub_date) as d_date, COUNT(pk_i_id) as num FROM %st_item WHERE dt_pub_date > '%s' GROUP BY DAY(dt_pub_date) ORDER BY dt_pub_date ASC", DB_TABLE_PREFIX, $from_date);
        }
        ...
        public function new_comments_count($from_date) {
            return $this->conn->osc_dbFetchResults("SELECT DATE(dt_pub_date) as d_date, COUNT(pk_i_id) as num FROM %st_item_comment WHERE dt_pub_date > '%s' GROUP BY DAY(dt_pub_date) ORDER BY dt_pub_date ASC", DB_TABLE_PREFIX, $from_date);
        }
