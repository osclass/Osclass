 ...
 * 
 */
class Search extends DAOSphinx
{
	/**
	 *
	 * @var type 
	...
	function __construct($expired = false) 
	{
		...
		$this->setTableName('OsClassItemIndex') ;
		$this->setFields( array('id') ) ;
		...
		if(!$expired) {
			// t_item 
			$this->addItemConditions("b_isexpired = 0");
		}
	...
	* NOT SUPPORTED
	...
	public function addCityArea($city_area = array())
	{
		if(is_array($city_area)) {
			foreach($city_area as $c) {
				...
				if($c!='') {
					if(is_numeric($c)) {
						$this->city_areas[] = sprintf("fk_i_city_area_id = %d ", $c);
					} else {
			...
			if($city_area!="") {
				...
				{
					$this->city_areas[] = sprintf("fk_i_city_area_id = %d ", $city_area);
				} else {
					$_city_area = CityArea::newInstance()->findByName($city_area) ;
					if( !empty($_city_area) ) 
					{
						$this->city_areas[] = sprintf("fk_i_city_area_id = %d ", $_city_area['pk_i_id']);
					} 
			/*		else 
					{
		...
		if(is_array($city)) {
			foreach($city as $c) {
				...
				if($c!='') {
					if(is_numeric($c)) 
					{
						$this->cities[] = sprintf("fk_i_city_id = %d ", $c);
					} else {
						$_city = City::newInstance()->findByName($c) ;
						if( !empty($_city) ) {
							$this->cities[] = sprintf("fk_i_city_id = %d ", $_city);
						} 
						/*else 
						{
			...
			if($city!="") {
				if(is_numeric($city)) 
				{
					$this->cities[] = sprintf("fk_i_city_id = %d ", $city);
				} 
				else 
				{
					...
					if( !empty($_city) ) 
					{
						$this->cities[] = sprintf("fk_i_city_id = %d ", $_city['pk_i_id']);
					} 
					/*else 
					{
		...
		if(is_array($region)) {
			foreach($region as $r) {
				...
				if($r!='') {
					if(is_numeric($r)) 
					{
						$this->regions[] = sprintf("fk_i_region_id = %d ", $r);
					} else {
						$_region = Region::newInstance()->findByName($r) ;
						if( !empty($_region) ) 
						{
							$this->regions[] = sprintf("fk_i_region_id = %d ", $_region['pk_i_id']);
						} 
						/*else 
						{
			...
			if($region!="") {
				...
				{
					$this->regions[] = sprintf("fk_i_region_id = %d ", $region);
				} 
				else 
				{
	...
	public function fromUser($id = NULL) 
	{
		if(is_array($id)) {
			...
			foreach($id as $_id) 
			{
				$ids[] = sprintf("fk_i_user_id = %d ", $_id);
			}
			$this->user_ids = $ids;
		} 
		else 
	...
	private function _fromUser()
	{
		...
		{	
			//$this->dao->where(sprintf("fk_i_user_id = %d ", $this->user_ids[0]));
			$this->dao->where(" fk_i_user_id IN ( ".implode(" , ", $this->user_ids)." ) ");
		} else {
			$this->dao->where(sprintf("fk_i_user_id = %d ", $this->user_ids));
		}
	}
	...
	private function _makeSQL($count = false,$premium = false) 
	{
		...
		if($this->withItemId) { 
			...
			//$this->dao->select('*');
			$this->dao->from('OsClassItemIndex');
			$this->dao->where('id', (int)$this->itemId);
		} else {  
			//$this->dao->select('*');
			...
			if ($this->withPattern ) {
				...
				$this->dao->where(sprintf("MATCH(@s_title %s)", $this->sPattern) );
				//$this->dao->where(sprintf("MATCH(d.s_title, d.s_description) AGAINST( IN BOOLEAN MODE)", $this->sPattern) );
			}
			...
			if($this->withCategoryId) {
				$this->dao->where('fk_i_category_id IN (' . implode(', ', $this->categories) .')' ) ;
			}
			if($this->withUserId) {
				$this->_fromUser();
			}
	...
	public function countAll()
	{
		if( is_null($this->total_results_table) ) {
			$result = $this->dao->query(sprintf('select count(*) as total from OsClassItemSource'));
			$row = $result->row();
	...
	{            
		...
		*/
		...
		if( is_array($category) && !empty ($category) ) {
			...
			$this->dao->where('fk_i_category_id IN ('.$listCategories.')');
			
		}
		
		// group by & order & limit
		$this->dao->orderBy(DB_TABLE_PREFIX.'pk_i_id', 'DESC');
