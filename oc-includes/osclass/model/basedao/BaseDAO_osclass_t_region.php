 ...
 */
    ...
    // Class: BaseDAO_osclass_t_region
    // Table: t_region
    // Database: osclass

    abstract class BaseDAO_osclass_t_region extends DAO
    {
        /* Attributes */
        var $DO ;
        var $aDO ;
        ...
        // Method that selects a row of "t_region" using the PK
        function selectBy_pk($pk_i_id, $aCampos="", $where="")
        {
            ...
            $sql .= "FROM " . $this->get_table_name('t_region') . " " ;
        ...
        // Method that selects all the rows of "t_region" that satisfies the WHERE conditions
        ...
        // Method that return the total of the rows of "t_region" that satisfies the WHERE conditions
        ...
        function insert($DO)
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                $can_insert = true ;
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_region') . " WHERE " . $where_select ;
            ...
            if ($can_insert)
            {
                $sql  = "INSERT INTO " . $this->get_table_name('t_region') . " SET " ;
		...
		function multiple_inserts($mDO) {
            ...
            $sql  = "INSERT INTO " . $this->get_table_name('t_region') . " (fk_c_country_code,s_name,b_active) VALUES " ;
            $cond = "" ;$primera = true;
        ...
        function updateBy_pk($DO, $where="")
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_region') . " WHERE (" . $where_select . ") AND NOT (pk_i_id = '" . addslashes($DO["pk_i_id"]) . "')";
            ...
            if ($can_update)
            {
                $sql  = "UPDATE " . $this->get_table_name('t_region') . " SET " ;
