 ...
 */
    ...
    // Class: BaseDAO_osclass_t_plugin_category
    // Table: t_plugin_category
    // Database: osclass

    abstract class BaseDAO_osclass_t_plugin_category extends DAO
    {
        /* Attributes */
        var $DO ;
        var $aDO ;
        ...
        /* Methods */
        // Method that selects all the rows of "t_plugin_category" that satisfies the WHERE conditions
        function selectAll($where = "", $order = "", $inicio = "", $total = "", $aCampos = "", $group = "")
        {
            ...
            $sql .= "FROM " . $this->get_table_name('t_plugin_category') . " " ;
        ...
        // Method that return the total of the rows of "t_plugin_category" that satisfies the WHERE conditions
        function selectTotal($where = "", $group = "")
        {
            $sql  = "SELECT count(s_plugin_name) AS total " ;
            if ($group) $sql.= ", " . $group . " " ;
            $sql .= "FROM " . $this->get_table_name('t_plugin_category') . " " ;
            if ($where) $sql .= " WHERE " . $where ;
            if ($group) $sql .= " GROUP BY " . $group ;
        ...
        function insert($DO)
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                $can_insert = true ;
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_plugin_category') . " WHERE " . $where_select ;
            ...
            if ($can_insert)
            {
                $sql  = "INSERT INTO " . $this->get_table_name('t_plugin_category') . " SET " ;

                $cond = "" ;
                if (isset($DO["s_plugin_name"]))
                {
                    if ($cond != "") $cond .= ", " ;
                       $cond .= "s_plugin_name = '" . addslashes($DO["s_plugin_name"]) . "'" ;
                }
		...
		function multiple_inserts($mDO) {
            ...
            $sql  = "INSERT INTO " . $this->get_table_name('t_plugin_category') . " (s_plugin_name,fk_i_category_id) VALUES " ;
            $cond = "" ;$primera = true;
		                ...
		                {
		                    if ($cond != "") $cond .= ", " ;
		                       $cond .= "'" . addslashes($DO["s_plugin_name"]) . "'" ; }
		                if (isset($DO["fk_i_category_id"]))
		                {
		                    if ($cond != "") $cond .= ", " ;
        ...
        function updateBy_pk($DO, $where="")
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_plugin_category') . " WHERE (" . $where_select . ") AND NOT ()";
            ...
            if ($can_update)
            {
                $sql  = "UPDATE " . $this->get_table_name('t_plugin_category') . " SET " ;
