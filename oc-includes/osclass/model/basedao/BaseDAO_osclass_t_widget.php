 ...
 */
    ...
    // Class: BaseDAO_osclass_t_widget
    // Table: t_widget
    // Database: osclass

    abstract class BaseDAO_osclass_t_widget extends DAO
    {
        /* Attributes */
        var $DO ;
        var $aDO ;
        ...
        // Method that selects a row of "t_widget" using the PK
        function selectBy_pk($pk_i_id, $aCampos="", $where="")
        {
            ...
            $sql .= "FROM " . $this->get_table_name('t_widget') . " " ;
        ...
        // Method that selects all the rows of "t_widget" that satisfies the WHERE conditions
        ...
        // Method that return the total of the rows of "t_widget" that satisfies the WHERE conditions
        ...
        function insert($DO)
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                $can_insert = true ;
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_widget') . " WHERE " . $where_select ;
            ...
            if ($can_insert)
            {
                $sql  = "INSERT INTO " . $this->get_table_name('t_widget') . " SET " ;
                ...
                if (isset($DO["s_location"]))
                {
                    if ($cond != "") $cond .= ", " ;
                       $cond .= "s_location = '" . addslashes($DO["s_location"]) . "'" ;
                }
                if (isset($DO["e_kind"]))
                {
                    if ($cond != "") $cond .= ", " ;
                       $cond .= "e_kind = '" . addslashes($DO["e_kind"]) . "'" ;
                }
                if (isset($DO["s_content"]))
                {
                    if ($cond != "") $cond .= ", " ;
                       $cond .= "s_content = '" . addslashes($DO["s_content"]) . "'" ;
                }
                $sql .= $cond ;
		...
		function multiple_inserts($mDO) {
            ...
            $sql  = "INSERT INTO " . $this->get_table_name('t_widget') . " (s_description,s_location,e_kind,s_content) VALUES " ;
            $cond = "" ;$primera = true;
		                ...
		                {
		                    if ($cond != "") $cond .= ", " ;
		                       $cond .= "'" . addslashes($DO["s_location"]) . "'" ; }
		                if (isset($DO["e_kind"]))
		                {
		                    if ($cond != "") $cond .= ", " ;
		                       $cond .= "'" . addslashes($DO["e_kind"]) . "'" ; }
		                if (isset($DO["s_content"]))
		                {
		                    if ($cond != "") $cond .= ", " ;
		                       $cond .= "'" . addslashes($DO["s_content"]) . "'" ; }
		                $sql .= $cond ;
        ...
        function updateBy_pk($DO, $where="")
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_widget') . " WHERE (" . $where_select . ") AND NOT (pk_i_id = '" . addslashes($DO["pk_i_id"]) . "')";
            ...
            if ($can_update)
            {
                $sql  = "UPDATE " . $this->get_table_name('t_widget') . " SET " ;
