 ...
 */
    ...
    // Class: BaseDAO_osclass_t_category_stats
    // Table: t_category_stats
    // Database: osclass

    abstract class BaseDAO_osclass_t_category_stats extends DAO
    {
        /* Attributes */
        var $DO ;
        var $aDO ;
        ...
        // Method that selects a row of "t_category_stats" using the PK
        function selectBy_pk($fk_i_category_id, $aCampos="", $where="")
        {
            $this->DO = "" ;
            $sql  = "SELECT " ;
            ...
            $sql .= "FROM " . $this->get_table_name('t_category_stats') . " " ;
        ...
        // Method that selects all the rows of "t_category_stats" that satisfies the WHERE conditions
        function selectAll($where = "", $order = "", $inicio = "", $total = "", $aCampos = "", $group = "")
        {
        	...
        	$is_set_pk_aCampos = ($aCampos == "" || (is_array($aCampos) && isset($aCampos["fk_i_category_id"])));
			
					
            while (!$rs->movenext())
            {
            	if($is_set_pk_aCampos) {
                	$this->aDO["pk_" . $rs->value("fk_i_category_id")] = $rs->fetch_array() ;
				} else {
        ...
        // Method that return the total of the rows of "t_category_stats" that satisfies the WHERE conditions
        ...
        function insert($DO)
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                $can_insert = true ;
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_category_stats') . " WHERE " . $where_select ;
            ...
            if ($can_insert)
            {
                $sql  = "INSERT INTO " . $this->get_table_name('t_category_stats') . " SET " ;
                ...
                if (isset($DO["i_num_items"]))
                {
                    if ($cond != "") $cond .= ", " ;
                    if ($DO["i_num_items"] !== "")     $cond .= "i_num_items = " . addslashes($DO["i_num_items"]) ;
                    else $cond .= "i_num_items = NULL" ;
                }
                $sql .= $cond ;
		...
		function multiple_inserts($mDO) {
            ...
            $sql  = "INSERT INTO " . $this->get_table_name('t_category_stats') . " (fk_i_category_id,i_num_items) VALUES " ;
            $cond = "" ;$primera = true;
		                ...
		                {
		                    if ($cond != "") $cond .= ", " ;
		                    if ($DO["i_num_items"] !== "")     $cond .= "" . addslashes($DO["i_num_items"]) ;
		                    else $cond .= " NULL" ; }
		                $sql .= $cond ;
        ...
        function updateBy_pk($DO, $where="")
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_category_stats') . " WHERE (" . $where_select . ") AND NOT (fk_i_category_id = '" . addslashes($DO["fk_i_category_id"]) . "')";
            ...
            if ($can_update)
            {
                $sql  = "UPDATE " . $this->get_table_name('t_category_stats') . " SET " ;
                ...
                $sql .= $cond . " WHERE fk_i_category_id = '" . addslashes($DO["fk_i_category_id"]) . "'" ;
                if ($where != "") $sql .= " AND " . $where ;
        ...
        function deleteBy_pk($fk_i_category_id, $where="")
        {
            $sql  = "DELETE " ;
            $sql .= "FROM " . $this->get_table_name('t_category_stats') . " " ;
            $sql .= "WHERE fk_i_category_id = '" . addslashes($fk_i_category_id) . "'" ;
