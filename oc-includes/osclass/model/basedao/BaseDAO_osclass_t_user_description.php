 ...
 */
    ...
    // Class: BaseDAO_osclass_t_user_description
    // Table: t_user_description
    // Database: osclass

    abstract class BaseDAO_osclass_t_user_description extends DAO
    {
        /* Attributes */
        var $DO ;
        var $aDO ;
        ...
        // Method that selects a row of "t_user_description" using the PK
        function selectBy_pk($fk_i_user_id, $fk_c_locale_code, $aCampos="", $where="")
        {
            $this->DO = "" ;
            $sql  = "SELECT " ;
            ...
            $sql .= "FROM " . $this->get_table_name('t_user_description') . " " ;
            $sql .= "WHERE fk_i_user_id = '" . addslashes($fk_i_user_id) . "' AND fk_c_locale_code = '" . addslashes($fk_c_locale_code) . "'" ;
        ...
        //Method that selects a row of "t_user_description" using "fk_i_user_id"
        function selectBy_fk_i_user_id($fk_i_user_id, $order = "", $aCampos="")
        {
            $sql  = "SELECT " ;
            ...
            while (!$rs->movenext())
            {
                $this->aDO["pk_" . $rs->value("fk_i_user_id") . "_" . $rs->value("fk_c_locale_code")] = $rs->fetch_array() ;
            }
        }
            

        //Method that selects a row of "t_user_description" using "fk_c_locale_code"
        ...
        // Method that selects all the rows of "t_user_description" that satisfies the WHERE conditions
        function selectAll($where = "", $order = "", $inicio = "", $total = "", $aCampos = "", $group = "")
        {
        	...
        	$is_set_pk_aCampos = ($aCampos == "" || (is_array($aCampos) && isset($aCampos["fk_i_user_id"]) && isset($aCampos["fk_c_locale_code"])));
        ...
        // Method that return the total of the rows of "t_user_description" that satisfies the WHERE conditions
        ...
        function insert($DO)
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                $can_insert = true ;
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_user_description') . " WHERE " . $where_select ;
            ...
            if ($can_insert)
            {
                $sql  = "INSERT INTO " . $this->get_table_name('t_user_description') . " SET " ;
                ...
                {
                    if ($cond != "") $cond .= ", " ;
                       $cond .= "s_info = '" . addslashes($DO["s_info"]) . "'" ;
                }
                $sql .= $cond ;
		...
		function multiple_inserts($mDO) {
            ...
            $sql  = "INSERT INTO " . $this->get_table_name('t_user_description') . " (fk_i_user_id,fk_c_locale_code,s_info) VALUES " ;
            $cond = "" ;$primera = true;
        ...
        function updateBy_pk($DO, $where="")
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_user_description') . " WHERE (" . $where_select . ") AND NOT (fk_i_user_id = '" . addslashes($DO["fk_i_user_id"]) . "' AND fk_c_locale_code = '" . addslashes($DO["fk_c_locale_code"]) . "')";
            ...
            if ($can_update)
            {
                $sql  = "UPDATE " . $this->get_table_name('t_user_description') . " SET " ;
                ...
                $sql .= $cond . " WHERE fk_i_user_id = '" . addslashes($DO["fk_i_user_id"]) . "' AND fk_c_locale_code = '" . addslashes($DO["fk_c_locale_code"]) . "'" ;
                if ($where != "") $sql .= " AND " . $where ;
        ...
        function deleteBy_pk($fk_i_user_id, $fk_c_locale_code, $where="")
        {
            $sql  = "DELETE " ;
            $sql .= "FROM " . $this->get_table_name('t_user_description') . " " ;
            $sql .= "WHERE fk_i_user_id = '" . addslashes($fk_i_user_id) . "' AND fk_c_locale_code = '" . addslashes($fk_c_locale_code) . "'" ;
        ...
        //Delete method using "fk_i_user_id"
        function deleteBy_fk_i_user_id($fk_i_user_id, $where="")
        {
            $sql  = "DELETE " ;
            $sql .= "FROM " . $this->get_table_name('t_user_description') . " " ;
            $sql .= "WHERE fk_i_user_id = '" . addslashes($fk_i_user_id) . "'" ;
