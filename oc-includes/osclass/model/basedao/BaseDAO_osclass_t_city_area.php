 ...
 */
    ...
    // Class: BaseDAO_osclass_t_city_area
    // Table: t_city_area
    // Database: osclass

    abstract class BaseDAO_osclass_t_city_area extends DAO
    {
        /* Attributes */
        var $DO ;
        var $aDO ;
        ...
        // Method that selects a row of "t_city_area" using the PK
        function selectBy_pk($pk_i_id, $aCampos="", $where="")
        {
            ...
            $sql .= "FROM " . $this->get_table_name('t_city_area') . " " ;
        ...
        // Method that selects all the rows of "t_city_area" that satisfies the WHERE conditions
        ...
        // Method that return the total of the rows of "t_city_area" that satisfies the WHERE conditions
        ...
        function insert($DO)
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                $can_insert = true ;
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_city_area') . " WHERE " . $where_select ;
            ...
            if ($can_insert)
            {
                $sql  = "INSERT INTO " . $this->get_table_name('t_city_area') . " SET " ;

                $cond = "" ;
                if (isset($DO["pk_i_id"]))
                {
                    if ($cond != "") $cond .= ", " ;
                    if ($DO["pk_i_id"] !== "")     $cond .= "pk_i_id = " . addslashes($DO["pk_i_id"]) ;
                    else $cond .= "pk_i_id = NULL" ;
                }
		...
		function multiple_inserts($mDO) {
            ...
            $sql  = "INSERT INTO " . $this->get_table_name('t_city_area') . " (pk_i_id,fk_i_city_id,s_name) VALUES " ;
            $cond = "" ;$primera = true;
		                ...
		                {
		                    if ($cond != "") $cond .= ", " ;
		                    if ($DO["pk_i_id"] !== "")     $cond .= "" . addslashes($DO["pk_i_id"]) ;
		                    else $cond .= " NULL" ; }
		                if (isset($DO["fk_i_city_id"]))
		                {
		                    if ($cond != "") $cond .= ", " ;
        ...
        function updateBy_pk($DO, $where="")
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_city_area') . " WHERE (" . $where_select . ") AND NOT (pk_i_id = '" . addslashes($DO["pk_i_id"]) . "')";
            ...
            if ($can_update)
            {
                $sql  = "UPDATE " . $this->get_table_name('t_city_area') . " SET " ;
