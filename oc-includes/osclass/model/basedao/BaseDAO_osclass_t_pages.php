 ...
 */
    ...
    // Class: BaseDAO_osclass_t_pages
    // Table: t_pages
    // Database: osclass

    abstract class BaseDAO_osclass_t_pages extends DAO
    {
        /* Attributes */
        var $DO ;
        var $aDO ;
        ...
        // Method that selects a row of "t_pages" using the PK
        function selectBy_pk($pk_i_id, $aCampos="", $where="")
        {
            ...
            $sql .= "FROM " . $this->get_table_name('t_pages') . " " ;
        ...
        // Method that selects all the rows of "t_pages" that satisfies the WHERE conditions
        ...
        // Method that return the total of the rows of "t_pages" that satisfies the WHERE conditions
        ...
        function insert($DO)
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                $can_insert = true ;
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_pages') . " WHERE " . $where_select ;
            ...
            if ($can_insert)
            {
                $sql  = "INSERT INTO " . $this->get_table_name('t_pages') . " SET " ;

                $cond = "" ;
                if (isset($DO["s_internal_name"]))
                {
                    if ($cond != "") $cond .= ", " ;
                       $cond .= "s_internal_name = '" . addslashes($DO["s_internal_name"]) . "'" ;
                }
                if (isset($DO["b_indelible"]))
                {
                    if ($cond != "") $cond .= ", " ;
                    if ($DO["b_indelible"] !== "")     $cond .= "b_indelible = " . addslashes($DO["b_indelible"]) ;
                    else $cond .= "b_indelible = NULL" ;
                }
		...
		function multiple_inserts($mDO) {
            ...
            $sql  = "INSERT INTO " . $this->get_table_name('t_pages') . " (s_internal_name,b_indelible,dt_pub_date,dt_mod_date) VALUES " ;
            $cond = "" ;$primera = true;
		                ...
		                {
		                    if ($cond != "") $cond .= ", " ;
		                       $cond .= "'" . addslashes($DO["s_internal_name"]) . "'" ; }
		                if (isset($DO["b_indelible"]))
		                {
		                    if ($cond != "") $cond .= ", " ;
		                    if ($DO["b_indelible"] !== "")     $cond .= "" . addslashes($DO["b_indelible"]) ;
		                    else $cond .= " NULL" ; }
		                if (isset($DO["dt_pub_date"]))
		                {
		                    if ($cond != "") $cond .= ", " ;
        ...
        function updateBy_pk($DO, $where="")
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_pages') . " WHERE (" . $where_select . ") AND NOT (pk_i_id = '" . addslashes($DO["pk_i_id"]) . "')";
            ...
            if ($can_update)
            {
                $sql  = "UPDATE " . $this->get_table_name('t_pages') . " SET " ;
