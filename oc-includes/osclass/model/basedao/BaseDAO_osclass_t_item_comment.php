 ...
 */
    ...
    // Class: BaseDAO_osclass_t_item_comment
    // Table: t_item_comment
    // Database: osclass

    abstract class BaseDAO_osclass_t_item_comment extends DAO
    {
        /* Attributes */
        var $DO ;
        var $aDO ;
        ...
        // Method that selects a row of "t_item_comment" using the PK
        function selectBy_pk($pk_i_id, $aCampos="", $where="")
        {
            ...
            $sql .= "FROM " . $this->get_table_name('t_item_comment') . " " ;
        ...
        // Method that selects all the rows of "t_item_comment" that satisfies the WHERE conditions
        ...
        // Method that return the total of the rows of "t_item_comment" that satisfies the WHERE conditions
        ...
        function insert($DO)
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                $can_insert = true ;
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_item_comment') . " WHERE " . $where_select ;
            ...
            if ($can_insert)
            {
                $sql  = "INSERT INTO " . $this->get_table_name('t_item_comment') . " SET " ;
                ...
                if (isset($DO["s_author_email"]))
                {
                    if ($cond != "") $cond .= ", " ;
                       $cond .= "s_author_email = '" . addslashes($DO["s_author_email"]) . "'" ;
                }
                if (isset($DO["s_body"]))
                {
                    if ($cond != "") $cond .= ", " ;
                       $cond .= "s_body = '" . addslashes($DO["s_body"]) . "'" ;
                }
		...
		function multiple_inserts($mDO) {
            ...
            $sql  = "INSERT INTO " . $this->get_table_name('t_item_comment') . " (fk_i_item_id,dt_pub_date,s_title,s_author_name,s_author_email,s_body,e_status,fk_i_user_id) VALUES " ;
            $cond = "" ;$primera = true;
		                ...
		                {
		                    if ($cond != "") $cond .= ", " ;
		                       $cond .= "'" . addslashes($DO["s_author_email"]) . "'" ; }
		                if (isset($DO["s_body"]))
		                {
		                    if ($cond != "") $cond .= ", " ;
		                       $cond .= "'" . addslashes($DO["s_body"]) . "'" ; }
		                if (isset($DO["e_status"]))
		                {
		                    if ($cond != "") $cond .= ", " ;
        ...
        function updateBy_pk($DO, $where="")
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_item_comment') . " WHERE (" . $where_select . ") AND NOT (pk_i_id = '" . addslashes($DO["pk_i_id"]) . "')";
            ...
            if ($can_update)
            {
                $sql  = "UPDATE " . $this->get_table_name('t_item_comment') . " SET " ;
