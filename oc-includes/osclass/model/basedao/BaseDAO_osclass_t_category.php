 ...
 */
    ...
    // Class: BaseDAO_osclass_t_category
    // Table: t_category
    // Database: osclass

    abstract class BaseDAO_osclass_t_category extends DAO
    {
        /* Attributes */
        var $DO ;
        var $aDO ;
        ...
        // Method that selects a row of "t_category" using the PK
        function selectBy_pk($pk_i_id, $aCampos="", $where="")
        {
            ...
            $sql .= "FROM " . $this->get_table_name('t_category') . " " ;
        ...
        // Method that selects all the rows of "t_category" that satisfies the WHERE conditions
        ...
        // Method that return the total of the rows of "t_category" that satisfies the WHERE conditions
        ...
        function insert($DO)
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                $can_insert = true ;
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_category') . " WHERE " . $where_select ;
            ...
            if ($can_insert)
            {
                $sql  = "INSERT INTO " . $this->get_table_name('t_category') . " SET " ;

                $cond = "" ;
                if (isset($DO["fk_i_parent_id"]))
                {
                    if ($cond != "") $cond .= ", " ;
                    if ($DO["fk_i_parent_id"] !== "")     $cond .= "fk_i_parent_id = " . addslashes($DO["fk_i_parent_id"]) ;
                    else $cond .= "fk_i_parent_id = NULL" ;
                }
                if (isset($DO["i_expiration_days"]))
                {
                    if ($cond != "") $cond .= ", " ;
                    if ($DO["i_expiration_days"] !== "")     $cond .= "i_expiration_days = " . addslashes($DO["i_expiration_days"]) ;
                    else $cond .= "i_expiration_days = NULL" ;
                }
                if (isset($DO["i_position"]))
                {
                    if ($cond != "") $cond .= ", " ;
                    if ($DO["i_position"] !== "")     $cond .= "i_position = " . addslashes($DO["i_position"]) ;
                    else $cond .= "i_position = NULL" ;
                }
                ...
                {
                    if ($cond != "") $cond .= ", " ;
                       $cond .= "s_icon = '" . addslashes($DO["s_icon"]) . "'" ;
                }
                $sql .= $cond ;
		...
		function multiple_inserts($mDO) {
            ...
            $sql  = "INSERT INTO " . $this->get_table_name('t_category') . " (fk_i_parent_id,i_expiration_days,i_position,b_enabled,s_icon) VALUES " ;
            $cond = "" ;$primera = true;
		                ...
		                {
		                    if ($cond != "") $cond .= ", " ;
		                    if ($DO["fk_i_parent_id"] !== "")     $cond .= "" . addslashes($DO["fk_i_parent_id"]) ;
		                    else $cond .= " NULL" ; }
		                if (isset($DO["i_expiration_days"]))
		                {
		                    if ($cond != "") $cond .= ", " ;
		                    if ($DO["i_expiration_days"] !== "")     $cond .= "" . addslashes($DO["i_expiration_days"]) ;
		                    else $cond .= " NULL" ; }
		                if (isset($DO["i_position"]))
		                {
		                    if ($cond != "") $cond .= ", " ;
		                    if ($DO["i_position"] !== "")     $cond .= "" . addslashes($DO["i_position"]) ;
		                    else $cond .= " NULL" ; }
		                if (isset($DO["b_enabled"]))
		                {
		                    if ($cond != "") $cond .= ", " ;
        ...
        function updateBy_pk($DO, $where="")
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_category') . " WHERE (" . $where_select . ") AND NOT (pk_i_id = '" . addslashes($DO["pk_i_id"]) . "')";
            ...
            if ($can_update)
            {
                $sql  = "UPDATE " . $this->get_table_name('t_category') . " SET " ;
