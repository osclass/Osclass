 ...
 */
    ...
    // Class: BaseDAO_osclass_t_country
    // Table: t_country
    // Database: osclass

    abstract class BaseDAO_osclass_t_country extends DAO
    {
        /* Attributes */
        var $DO ;
        var $aDO ;
        ...
        // Method that selects a row of "t_country" using the PK
        function selectBy_pk($pk_c_code, $fk_c_locale_code, $aCampos="", $where="")
        {
            $this->DO = "" ;
            $sql  = "SELECT " ;
            ...
            $sql .= "FROM " . $this->get_table_name('t_country') . " " ;
            $sql .= "WHERE pk_c_code = '" . addslashes($pk_c_code) . "' AND fk_c_locale_code = '" . addslashes($fk_c_locale_code) . "'" ;
        ...
        //Method that selects a row of "t_country" using "pk_c_code"
        function selectBy_pk_c_code($pk_c_code, $order = "", $aCampos="")
        {
            $sql  = "SELECT " ;
            ...
            while (!$rs->movenext())
            {
                $this->aDO["pk_" . $rs->value("pk_c_code") . "_" . $rs->value("fk_c_locale_code")] = $rs->fetch_array() ;
            }
        }
            

        //Method that selects a row of "t_country" using "fk_c_locale_code"
        ...
        // Method that selects all the rows of "t_country" that satisfies the WHERE conditions
        function selectAll($where = "", $order = "", $inicio = "", $total = "", $aCampos = "", $group = "")
        {
        	...
        	$is_set_pk_aCampos = ($aCampos == "" || (is_array($aCampos) && isset($aCampos["pk_c_code"]) && isset($aCampos["fk_c_locale_code"])));
        ...
        // Method that return the total of the rows of "t_country" that satisfies the WHERE conditions
        ...
        function insert($DO)
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                $can_insert = true ;
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_country') . " WHERE " . $where_select ;
            ...
            if ($can_insert)
            {
                $sql  = "INSERT INTO " . $this->get_table_name('t_country') . " SET " ;
		...
		function multiple_inserts($mDO) {
            ...
            $sql  = "INSERT INTO " . $this->get_table_name('t_country') . " (pk_c_code,fk_c_locale_code,s_name) VALUES " ;
            $cond = "" ;$primera = true;
        ...
        function updateBy_pk($DO, $where="")
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_country') . " WHERE (" . $where_select . ") AND NOT (pk_c_code = '" . addslashes($DO["pk_c_code"]) . "' AND fk_c_locale_code = '" . addslashes($DO["fk_c_locale_code"]) . "')";
            ...
            if ($can_update)
            {
                $sql  = "UPDATE " . $this->get_table_name('t_country') . " SET " ;
                ...
                $sql .= $cond . " WHERE pk_c_code = '" . addslashes($DO["pk_c_code"]) . "' AND fk_c_locale_code = '" . addslashes($DO["fk_c_locale_code"]) . "'" ;
                if ($where != "") $sql .= " AND " . $where ;
        ...
        function deleteBy_pk($pk_c_code, $fk_c_locale_code, $where="")
        {
            $sql  = "DELETE " ;
            $sql .= "FROM " . $this->get_table_name('t_country') . " " ;
            $sql .= "WHERE pk_c_code = '" . addslashes($pk_c_code) . "' AND fk_c_locale_code = '" . addslashes($fk_c_locale_code) . "'" ;
        ...
        //Delete method using "pk_c_code"
        function deleteBy_pk_c_code($pk_c_code, $where="")
        {
            $sql  = "DELETE " ;
            $sql .= "FROM " . $this->get_table_name('t_country') . " " ;
            $sql .= "WHERE pk_c_code = '" . addslashes($pk_c_code) . "'" ;
