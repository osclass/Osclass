 ...
 */
    ...
    // Class: BaseDAO_osclass_t_item_resource
    // Table: t_item_resource
    // Database: osclass

    abstract class BaseDAO_osclass_t_item_resource extends DAO
    {
        /* Attributes */
        var $DO ;
        var $aDO ;
        ...
        // Method that selects a row of "t_item_resource" using the PK
        function selectBy_pk($pk_i_id, $aCampos="", $where="")
        {
            ...
            $sql .= "FROM " . $this->get_table_name('t_item_resource') . " " ;
        ...
        // Method that selects all the rows of "t_item_resource" that satisfies the WHERE conditions
        ...
        // Method that return the total of the rows of "t_item_resource" that satisfies the WHERE conditions
        ...
        function insert($DO)
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                $can_insert = true ;
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_item_resource') . " WHERE " . $where_select ;
            ...
            if ($can_insert)
            {
                $sql  = "INSERT INTO " . $this->get_table_name('t_item_resource') . " SET " ;
                ...
                if (isset($DO["s_extension"]))
                {
                    if ($cond != "") $cond .= ", " ;
                       $cond .= "s_extension = '" . addslashes($DO["s_extension"]) . "'" ;
                }
                if (isset($DO["s_content_type"]))
                {
                    if ($cond != "") $cond .= ", " ;
                       $cond .= "s_content_type = '" . addslashes($DO["s_content_type"]) . "'" ;
                }
                if (isset($DO["s_path"]))
                {
                    if ($cond != "") $cond .= ", " ;
                       $cond .= "s_path = '" . addslashes($DO["s_path"]) . "'" ;
                }
                $sql .= $cond ;
		...
		function multiple_inserts($mDO) {
            ...
            $sql  = "INSERT INTO " . $this->get_table_name('t_item_resource') . " (fk_i_item_id,s_name,s_extension,s_content_type,s_path) VALUES " ;
            $cond = "" ;$primera = true;
		                ...
		                {
		                    if ($cond != "") $cond .= ", " ;
		                       $cond .= "'" . addslashes($DO["s_extension"]) . "'" ; }
		                if (isset($DO["s_content_type"]))
		                {
		                    if ($cond != "") $cond .= ", " ;
		                       $cond .= "'" . addslashes($DO["s_content_type"]) . "'" ; }
		                if (isset($DO["s_path"]))
		                {
		                    if ($cond != "") $cond .= ", " ;
		                       $cond .= "'" . addslashes($DO["s_path"]) . "'" ; }
		                $sql .= $cond ;
        ...
        function updateBy_pk($DO, $where="")
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_item_resource') . " WHERE (" . $where_select . ") AND NOT (pk_i_id = '" . addslashes($DO["pk_i_id"]) . "')";
            ...
            if ($can_update)
            {
                $sql  = "UPDATE " . $this->get_table_name('t_item_resource') . " SET " ;
