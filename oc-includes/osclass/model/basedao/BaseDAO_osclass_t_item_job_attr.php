 ...
 */
    ...
    // Class: BaseDAO_osclass_t_item_job_attr
    // Table: t_item_job_attr
    // Database: osclass

    abstract class BaseDAO_osclass_t_item_job_attr extends DAO
    {
        /* Attributes */
        var $DO ;
        var $aDO ;
        ...
        // Method that selects a row of "t_item_job_attr" using the PK
        function selectBy_pk($fk_i_item_id, $aCampos="", $where="")
        {
            ...
            $sql .= "FROM " . $this->get_table_name('t_item_job_attr') . " " ;
        ...
        // Method that selects all the rows of "t_item_job_attr" that satisfies the WHERE conditions
        ...
        // Method that return the total of the rows of "t_item_job_attr" that satisfies the WHERE conditions
        ...
        function insert($DO)
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                $can_insert = true ;
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_item_job_attr') . " WHERE " . $where_select ;
            ...
            if ($can_insert)
            {
                $sql  = "INSERT INTO " . $this->get_table_name('t_item_job_attr') . " SET " ;
                ...
                if (isset($DO["e_relation"]))
                {
                    if ($cond != "") $cond .= ", " ;
                       $cond .= "e_relation = '" . addslashes($DO["e_relation"]) . "'" ;
                }
                if (isset($DO["s_company_name"]))
                {
                    if ($cond != "") $cond .= ", " ;
                       $cond .= "s_company_name = '" . addslashes($DO["s_company_name"]) . "'" ;
                }
                if (isset($DO["e_position_type"]))
                {
                    if ($cond != "") $cond .= ", " ;
                       $cond .= "e_position_type = '" . addslashes($DO["e_position_type"]) . "'" ;
                }
                if (isset($DO["i_salary_min"]))
                {
                    if ($cond != "") $cond .= ", " ;
                    if ($DO["i_salary_min"] !== "")     $cond .= "i_salary_min = " . addslashes($DO["i_salary_min"]) ;
                    else $cond .= "i_salary_min = NULL" ;
                }
                if (isset($DO["i_salary_max"]))
                {
                    if ($cond != "") $cond .= ", " ;
                    if ($DO["i_salary_max"] !== "")     $cond .= "i_salary_max = " . addslashes($DO["i_salary_max"]) ;
                    else $cond .= "i_salary_max = NULL" ;
                }
                if (isset($DO["e_salary_period"]))
                {
                    if ($cond != "") $cond .= ", " ;
                       $cond .= "e_salary_period = '" . addslashes($DO["e_salary_period"]) . "'" ;
                }
                $sql .= $cond ;
		...
		function multiple_inserts($mDO) {
            ...
            $sql  = "INSERT INTO " . $this->get_table_name('t_item_job_attr') . " (fk_i_item_id,e_relation,s_company_name,e_position_type,i_salary_min,i_salary_max,e_salary_period) VALUES " ;
            $cond = "" ;$primera = true;
		                ...
		                {
		                    if ($cond != "") $cond .= ", " ;
		                       $cond .= "'" . addslashes($DO["e_relation"]) . "'" ; }
		                if (isset($DO["s_company_name"]))
		                {
		                    if ($cond != "") $cond .= ", " ;
		                       $cond .= "'" . addslashes($DO["s_company_name"]) . "'" ; }
		                if (isset($DO["e_position_type"]))
		                {
		                    if ($cond != "") $cond .= ", " ;
		                       $cond .= "'" . addslashes($DO["e_position_type"]) . "'" ; }
		                if (isset($DO["i_salary_min"]))
		                {
		                    if ($cond != "") $cond .= ", " ;
		                    if ($DO["i_salary_min"] !== "")     $cond .= "" . addslashes($DO["i_salary_min"]) ;
		                    else $cond .= " NULL" ; }
		                if (isset($DO["i_salary_max"]))
		                {
		                    if ($cond != "") $cond .= ", " ;
		                    if ($DO["i_salary_max"] !== "")     $cond .= "" . addslashes($DO["i_salary_max"]) ;
		                    else $cond .= " NULL" ; }
		                if (isset($DO["e_salary_period"]))
		                {
		                    if ($cond != "") $cond .= ", " ;
		                       $cond .= "'" . addslashes($DO["e_salary_period"]) . "'" ; }
		                $sql .= $cond ;
        ...
        function updateBy_pk($DO, $where="")
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_item_job_attr') . " WHERE (" . $where_select . ") AND NOT (fk_i_item_id = '" . addslashes($DO["fk_i_item_id"]) . "')";
            ...
            if ($can_update)
            {
                $sql  = "UPDATE " . $this->get_table_name('t_item_job_attr') . " SET " ;
