 ...
 */
    ...
    // Class: BaseDAO_osclass_t_cron
    // Table: t_cron
    // Database: osclass

    abstract class BaseDAO_osclass_t_cron extends DAO
    {
        /* Attributes */
        var $DO ;
        var $aDO ;
        ...
        /* Methods */
        // Method that selects all the rows of "t_cron" that satisfies the WHERE conditions
        function selectAll($where = "", $order = "", $inicio = "", $total = "", $aCampos = "", $group = "")
        {
            ...
            $sql .= "FROM " . $this->get_table_name('t_cron') . " " ;
        ...
        // Method that return the total of the rows of "t_cron" that satisfies the WHERE conditions
        function selectTotal($where = "", $group = "")
        {
            $sql  = "SELECT count(e_type) AS total " ;
            if ($group) $sql.= ", " . $group . " " ;
            $sql .= "FROM " . $this->get_table_name('t_cron') . " " ;
            if ($where) $sql .= " WHERE " . $where ;
            if ($group) $sql .= " GROUP BY " . $group ;
        ...
        function insert($DO)
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                $can_insert = true ;
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_cron') . " WHERE " . $where_select ;
            ...
            if ($can_insert)
            {
                $sql  = "INSERT INTO " . $this->get_table_name('t_cron') . " SET " ;
                ...
                if (isset($DO["d_last_exec"]))
                {
                    if ($cond != "") $cond .= ", " ;
                    if ($DO["d_last_exec"] !== "")    $cond .= "d_last_exec = '" . addslashes($DO["d_last_exec"]) . "'" ;
                    else $cond .= "d_last_exec = NULL" ;
                }
                if (isset($DO["d_next_exec"]))
                {
                    if ($cond != "") $cond .= ", " ;
                    if ($DO["d_next_exec"] !== "")    $cond .= "d_next_exec = '" . addslashes($DO["d_next_exec"]) . "'" ;
                    else $cond .= "d_next_exec = NULL" ;
                }
                $sql .= $cond ;
		...
		function multiple_inserts($mDO) {
            ...
            $sql  = "INSERT INTO " . $this->get_table_name('t_cron') . " (e_type,d_last_exec,d_next_exec) VALUES " ;
            $cond = "" ;$primera = true;
		                ...
		                {
		                    if ($cond != "") $cond .= ", " ;
		                    if ($DO["d_last_exec"] !== "")    $cond .= "'" . addslashes($DO["d_last_exec"]) . "'" ;
		                    else $cond .= " NULL" ; }
		                if (isset($DO["d_next_exec"]))
		                {
		                    if ($cond != "") $cond .= ", " ;
		                    if ($DO["d_next_exec"] !== "")    $cond .= "'" . addslashes($DO["d_next_exec"]) . "'" ;
		                    else $cond .= " NULL" ; }
		                $sql .= $cond ;
        ...
        function updateBy_pk($DO, $where="")
        {
            ...
            if (is_array($this->noDuplicateFields) || is_array($this->noDuplicateUniqueKey))
            {
                ...
                if ($where_select != "") {
                    $sql = "SELECT * FROM " . $this->get_table_name('t_cron') . " WHERE (" . $where_select . ") AND NOT ()";
            ...
            if ($can_update)
            {
                $sql  = "UPDATE " . $this->get_table_name('t_cron') . " SET " ;
