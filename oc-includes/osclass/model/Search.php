    ...
    class Search extends DAO
    {
        ...
        public function __construct($expired = false) {
            ...
            if(!$expired) {
                ...
                $this->addConditions(sprintf(" (%st_item.b_premium = 1 || %st_category.i_expiration_days = 0 ||DATEDIFF(%st_item.dt_pub_date,'%s') < %st_category.i_expiration_days) ", DB_TABLE_PREFIX, DB_TABLE_PREFIX, DB_TABLE_PREFIX, date('Y-m-d H:i:s'), DB_TABLE_PREFIX));
...
//                $this->order_column = sprintf("query.$o_c", DB_TABLE_PREFIX);
        ...
        public function getPremiums($max = 2) {
            ...
            $this->addField(sprintf('(sum(%st_item_stats.i_num_premium_views) + sum(%st_item_stats.i_num_premium_views)*RAND()*0.7 + DATEDIFF(%st_item.dt_pub_date,\'%s\')*0.3) as order_premium_views', DB_TABLE_PREFIX, DB_TABLE_PREFIX, DB_TABLE_PREFIX, date('Y-m-d H:i:s')));
