...
<html xmlns="http://www.w3.org/1999/xhtml" dir="ltr" lang="en-US">
    ...
    <body>
        ...
        <div class="content add_item">
            ...
            <form action="<?php echo osc_base_url(true); ?>" method="post" enctype="multipart/form-data" onSubmit="return checkForm()">
                ...
                <input type="hidden" name="catId" value="<?php echo  Params::getParam('catId'); ?>" />
                    ...
                    <div class="box general_info">
                        ...
                        <div class="row">
                            ...
                            <?php foreach($categories as $c): ?>
                                <option value="<?php echo $c['pk_i_id']; ?>" <?php if((Params::getParam('catId') != '' ) && Params::getParam('catId')==$c['pk_i_id'] ) { echo "selected"; } ?>><?php echo $c['s_name']; ?></option>
                                <?php foreach($c['categories'] as $sc): ?>
                                    <option style="padding-left: 20px;" value="<?php echo $sc['pk_i_id']; ?>" <?php if(( Params::getParam('catId') != '') && Params::getParam('catId')==$sc['pk_i_id']) { echo "selected"; } ?>><?php echo $sc['s_name']; ?></option>
                                <?php endforeach; ?>
                            <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        $locale = $locales[0]; ?>
                                        <div class="row">
                                            <label for="title"><?php _e('Title'); ?></label>
                                                <input type="text" name="title[<?php echo @$locale['pk_c_code']; ?>]" id="title" />
                                    </div>
                                    <div class="row">
                                                <label for="description"><?php _e('Description'); ?></label>
                                                <textarea name="description[<?php echo @$locale['pk_c_code']; ?>]" id="description"></textarea>
                                        </div>
                       <?php } else {?>
                                    <div class="tabber">
        ...
        <script src="<?php echo osc_base_path();?>/oc-includes/js/tabber-minimized.js"></script>
        <link type="text/css" href="<?php echo osc_base_path();?>/oc-includes/css/tabs.css" media="screen" rel="stylesheet" />
    ...
    <script type="text/javascript">
        ...
        $("#regionId").change(function(){
            var pk_c_code = $(this).val();
            var url = '<?php echo osc_base_path() . "/oc-includes/osclass/ajax/city.php?regionId="; ?>' + pk_c_code;
            var result = '';
