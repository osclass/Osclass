...
$(document).ready(function(){
    $(".sub_button").click(function(){
        $.post('<?php echo osc_base_url() ; ?>oc-includes/osclass/ajax/alerts.php', {email:$("#email").val(), userid:$("#userId").val(), alert:$("#alert").val()}, function(data){ if(data==1) { alert('<?php _e('You have been subscribe correctly to the alert'); ?>'); } else { alert('<?php _e('There was a problem with the alert');?>');}; });
        return false;
    });
});
</script>
...
<div name="alert-form">
    <?php _e('Subscribe to this search'); ?><br />
